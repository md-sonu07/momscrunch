...
const ProductCard = ({ product, variant = "vertical" }) => {
    ...
    const startingPrice = parseFloat(product.starting_from || 0);
    const price = discountPrice > 0 ? discountPrice : (product.price || product.product_price || startingPrice);
    const oldPrice = (discountPrice > 0 && startingPrice > 0) ? startingPrice : (product.oldPrice || product.product_old_price || null);
    const discount = (oldPrice && price && oldPrice > price) ? Math.round(((oldPrice - price) / oldPrice) * 100) : 0;
